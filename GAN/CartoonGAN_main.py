...
def get_args():
    ...
    parser.add_argument(
        "--test", action="store_true", help="Use this argument to test generator"
    )
    ...
    parser.add_argument(
        "--photo_image_dir", default=Config.photo_image_dir, help="Path to photo images"
    )
...
def main():
    ...
    if args.test:
        ...
        test_images = load_image_dataloader(
            root_dir=args.test_image_path, batch_size=1, shuffle=False
        )
        ...
        generate_and_save_images(generator, test_images, "generated_images/")
        ...
        # load dataloaders
        photo_images = load_image_dataloader(
            root_dir=args.photo_image_dir, batch_size=args.batch_size
        )
        animation_images = load_image_dataloader(
            root_dir=args.animation_image_dir, batch_size=args.batch_size
        )
        edge_smoothed_images = load_image_dataloader(
            root_dir=args.edge_smoothed_image_dir, batch_size=args.batch_size
        )
        ...
        trainer = CartoonGANTrainer(
            ...
            lsgan=args.use_modified_model,
        )
        if args.model_path:
            trainer.load_checkpoint(args.model_path)

        print("Start Training...")
        loss_D_hist, loss_G_hist, loss_content_hist = trainer.train(
